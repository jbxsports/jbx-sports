...
async function enviarEmailConfirmacao(email, nome, item, dataEvento) {
  ...
  try {
    ...
    console.log('[mp-webhook] E-mail confirmação enviado para', email.slice(0,4) + '***');
  } catch(e) {
...
async function marcarComoPago(pedido) {
  try {
    ...
    console.log('[mp-webhook] confirmar_pagamento status:', res.status, text);
  } catch(e) {
...
module.exports = async (req, res) => {
  ...
  // Nos interessa: payment com status approved, ou merchant_order approved
  let pedido   = null;
  let itens    = [];
  let eventoNome = '';
  ...
  try {
    ...
    if (type === 'payment' && data?.id) {
      ...
      console.log('[mp-webhook] payment status:', payment.status, '| external_ref:', payment.external_reference);
  ...
  console.log('[mp-webhook] processando pedido:', pedido, '| itens:', itens.length);
  ...
  return res.status(200).json({ received: true, pedido, itens: itens.length });
};
